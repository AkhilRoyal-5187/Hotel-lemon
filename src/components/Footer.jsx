...
// Import Heroicons or other icon library
import { PhoneIcon, ClockIcon, EnvelopeIcon, MapPinIcon } from '@heroicons/react/24/outline'; // Using outline icons
...
const Footer = () => {
    ...
    // Effect to control the main footer animation based on inView status
    useEffect(() => {
        if (inView) {
    ...
    // Animation variants for the main footer content container
    const containerVariants = {
        hidden: { opacity: 0 },
        visible: {
            opacity: 1,
            transition: {
                staggerChildren: 0.1, // Stagger delay for immediate children (logo, contact, social, links)
                delayChildren: 0.2, // Delay before the first child starts animating
            },
        },
    };

    // Animation variants for immediate children of the main container
    const itemVariants = {
        hidden: { opacity: 0, y: 30 },
        visible: { opacity: 1, y: 0, transition: { duration: 0.6, ease: 'easeOut' } },
    };

    // Variants for individual quick links and social links for staggering within their sections
    ...
    return (
        // Main footer element with theme background and padding
        <footer className='py-[60px] px-4 bg-gradient-to-br from-gray-900 to-[#9b111e] mx-2 rounded-3xl text-white font-Lora mt-8'> {/* Applied theme styles */}
            ...
            <motion.div
                ref={ref} // Attach the inView ref here
                className='max-w-7xl mx-auto flex flex-col items-center gap-10 text-center' // Applied layout and spacing
                variants={containerVariants} // Apply container animation variants
                initial="hidden" // Set initial state
                animate={controls} // Control animation with the main controls
            >
                {/* Logo */}
                <motion.a
                    href="/"
                    className='inline-block leading-none' // Keep inline-block for layout
                    aria-label="Homepage"
                    variants={itemVariants} // Animate with item variants
                >
                    <LogoWhite className="w-[100px] h-auto" /> {/* Adjust logo size if needed */}
                </motion.a>

                {/* Contact Section */}
                {/* Animate this section with item variants */}
                <motion.div
                    className='flex flex-wrap justify-center gap-4 text-base' // Applied layout, spacing, and text size
                    variants={itemVariants} // Animate with item variants
                >
                    ...
                    >
                        <PhoneIcon className='w-5 h-5 text-yellow-400' /> {/* Styled Icon */}
                     ...
                     <motion.div
                        ...
                        variants={linkItemVariants} // Animate individual contact items
                    >
                        <ClockIcon className='w-5 h-5 text-yellow-400' /> {/* Styled Icon */}
                     ...
                     <motion.div
                        className='flex items-center gap-2 px-4 py-2 bg-white/10 rounded-lg border border-white/15 transition-all duration-300 hover:bg-white/20 hover:border-white/30 cursor-pointer' // Applied elegant theme styles
                         variants={linkItemVariants} // Animate individual contact items
                    >
                        <EnvelopeIcon className='w-5 h-5 text-yellow-400' /> {/* Styled Icon */}
                     ...
                     <motion.div
                        className='flex items-center gap-2 px-4 py-2 bg-white/10 rounded-lg border border-white/15 transition-all duration-300 hover:bg-white/20 hover:border-white/30 cursor-pointer' // Applied elegant theme styles
                         variants={linkItemVariants} // Animate individual contact items
                    >
                        <MapPinIcon className='w-5 h-5 text-yellow-400' /> {/* Styled Icon */}
                ...
                {/* Social Links Section */}
                 {/* Animate this section with item variants */}
                <motion.div
                    className='flex justify-center gap-6 text-2xl' // Applied layout, spacing, and text size
                    variants={itemVariants} // Animate with item variants
                >
                    {/* Individual Social Links with hover animation */}
                    <motion.a
                        href="#"
                    ...
                    >
                        {/* Replace with actual social icons if available, using Heroicons as placeholders */}
